...
```html
<template>
  <ly-button type="text" @click="open">点击打开 Message Box</ly-button>
</template>
...
<setup>
  ...
  export default defineComponent({
    setup() {
      ...
      const open = () => {
        LyMessageBox.alert('这是一段内容', '标题名称', {
          confirmButtonText: '确定',
          callback: (action) => {
            LyMessage({
              type: 'info',
...
<setup>
  ...
  export default defineComponent({
    setup() {
      ...
      const open = () => {
        LyMessageBox.confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        })
...
<setup>
  ...
  export default defineComponent({
    setup() {
      ...
      const open = () => {
        LyMessageBox.alert('<strong>这是 <i>HTML</i> 片段</strong>', 'HTML 片段', {
          dangerouslyUseHTMLString: true,
        });
      };
...
<setup>
  ...
  export default defineComponent({
    setup() {
      ...
      const open = () => {
        LyMessageBox.confirm('检测到未保存的内容，是否在离开页面前保存修改？', '确认信息', {
          distinguishCancelAndClose: true,
          confirmButtonText: '保存',
          cancelButtonText: '放弃修改',
        })
...
:::demo 将 `center` 设置为 `false` 即可开启右布局
...
那么对应于上述四个全局方法的调用方法依次为：LyMessageBox, LyMessageBox.alert, LyMessageBox.confirm 和 LyMessageBox.prompt，调用参数与全局方法相同。
