...
```html
<template>
  <ly-button @click="add">添加元素</ly-button>
  <ly-button @click="delete">删除元素</ly-button>
  <ly-scrollbar max-height="400px">
...
```html
<template>
  <ly-scrollbar ref="scrollbar" height="400px" always @scroll="scroll">
    <div ref="inner">
      <p class="item" v-for="item in 20">{{ item }}</p>
    </div>
  </ly-scrollbar>
