...
```html
<ly-tree :props="props" :load="loadNode" lazy show-checkbox> </ly-tree>
...
<div class="buttons">
  <ly-button @click="getCheckedNodes">通过 node 获取</ly-button>
  <ly-button @click="getCheckedKeys">通过 key 获取</ly-button>
  <ly-button @click="setCheckedNodes">通过 node 设置</ly-button>
  <ly-button @click="setCheckedKeys">通过 key 设置</ly-button>
  <ly-button @click="resetChecked">清空</ly-button>
</div>
...
export default {
    setup() {
        ...
        function getCheckedNodes() {
            console.log(proxy.$refs.tree.getCheckedNodes());
        }
        function getCheckedKeys() {
            console.log(proxy.$refs.tree.getCheckedKeys());
        }
        ...
        function setCheckedKeys() {
            proxy.$refs.tree.setCheckedKeys([3]);
        }
...
export default {
    setup() {
        const data = ref(JSON.parse(JSON.stringify(val)));
...
```html
<ly-input placeholder="输入关键字进行过滤" v-model="filterText"> </ly-input>
...
<!--
<setup>
    import { watch, getCurrentInstance, reactive, ref } from 'vue';
    export default {
        setup() {
            ...
            watch(
                () => filterText.value,
                (newVal) => proxy.$refs.tree.filter(newVal)
            );
            const defaultProps = reactive({
                children: 'children',
                label: 'label',
...
| splitState            | 勾选节点的时候是否改变该节点的父级状态, 默认情况下会影响                                                                                   | boolean                                | —      | true   |
| search-one            | 勾选节点的时候，只选一级的下级                                                                                                             | boolean                                | —      | false  |
| search-all            | 勾选节点的时候，选中所有下级                                                                                                               | boolean                                | —      | true   |
