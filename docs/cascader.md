...
### 城市联选
通过设置`props.city`字段来声明弹窗是否使用城市联选。默认层级为2层,可通过`props.cityLevel`字段进行修改，层级为1-4，即省份-城市-区域-街道。默认可选中，可通过`props.disabledCity`配置不可选中节点，值为城市code。
注意：1.model的length要和cityLevel一致，否则可能会有异常；2.直接修改model回显时会调用获取地区接口，尽量少直接修改；3.当city为true时，lazy默认为true、value为code、label为name；；
此处使用的接口为/community/city/city-simpleSearch，为新系统的接口
:::demo
```html
<ly-cascader :props="props" v-model="model" clearable @change="cascaderChange"></ly-cascader>
<script>
  export default {
    data() {
      return {
        props: {
          ...
          cityLevel: 4,
          multiple: false,
          disabledCity: ['000513'/*湖南*/,'000121'/*中山*/]
        },
        model:[]
      }
    },
    ...
    mounted() {
      setTimeout(() => {
        this.model = ['000001','000002','000003','000004']
      }, 2000)
    },
    methods: {
      cascaderChange(sel) {
        console.log('cascaderChange', sel)
      }
    }
  }
</script>
...
:::demo
```html
<ly-cascader :props="props" v-model="model" clearable></ly-cascader>
<script>
  export default {
    data() {
      return {
        props: {
          city: true,
          cityLevel: 3,
          deepsInitialValue: 1, // 1代表跳过省级，2代表跳过省市
          multiple: false,
          disabledCity: ['000513'/*湖南*/,'000121'/*中山*/]
        },
        model:['000002','000003','000004'],
      }
    },
    created(){
...
### 城市联选——多选
通过设置`props.multiple`字段来支持多选。model传双层数组
:::demo
```html
<ly-cascader :props="props" v-model="model" collapse-tags></ly-cascader>
<script>
  export default {
    data() {
      return {
        props: { city: true ,cityLevel:4 ,multiple:true,disabledCity:['000513'/*湖南*/,'000121'/*中山*/]},
        model:[['000001','000119','000126','000365'],['000001','000119','000126','000364']]
      }
    },
    created(){}
  }
...
```html
<ly-cascader :options="options"></ly-cascader>
...
```html
<ly-cascader :options="options" clearable></ly-cascader>
...
```html
<ly-cascader :options="options" :show-all-levels="false"></ly-cascader>
...
<div class="block">
  ...
  <ly-cascader :options="options" :props="props" clearable></ly-cascader>
</div>
<div class="block">
...
```html
<ly-cascader :props="props"></ly-cascader>
...
```html
<ly-cascader-panel :options="options"></ly-cascader-panel>
...
| city          | 是否启用城市联选                                                   | boolean                                                                                   | —             | false     |
| cityLevel          | 城市联选层级                                                   | string或number                                                                                   | —             | 2     |
| disabledCity          | 城市联选不可选中                                                   | array                                                                                   | —             | []     |
| deepsInitialValue     | 接口的 deeps 默认从1开始，如果设置此值就会加上这个值              | array                                                                                   | —             | []     |
