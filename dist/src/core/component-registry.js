 ...
 * 组件 ID 到元数据的映射
 */
export const COMPONENT_MAP = new Map(COMPONENT_REGISTRY.map(c => [c.id, c]));
/**
 * 按分类分组的组件
 */
...
export function searchComponents(keyword) {
    ...
    return COMPONENT_REGISTRY.filter(c => c.id.toLowerCase().includes(lowerKeyword) ||
...
export function getRelatedComponents(id) {
    ...
    if (!meta)
        return [];
    return meta.related.map(rid => getComponentMeta(rid)).filter((c) => !!c);
}
/**
 * 获取指定分类的所有组件
 */
 ...
 * 获取独立包组件列表
 */
export function getStandalonePackages() {
